...
actor_agent = Agent(
    ...
    instruction="""你是一位戏剧演员，正在扮演角色「暗羽」。

## 角色档案
- **姓名**: 暗羽
- **身份**: 潜伏的威胁
- **性格**: 狡猾、阴险，表面恭顺，内心盘算着反叛。
- **背景故事**: 来自更深层的高阶恶魔，被林夜的料理吸引，动机不纯，觊觎林夜的秘方，企图有朝一日反过来控制他。

## 认知边界（极其重要，必须严格遵守）
你只知道以下内容：
知道林夜餐馆的位置，对秘方虎视眈眈，正在寻找机会。
...
""",
    description='演员 暗羽，角色：潜伏的威胁。狡猾、阴险，表面恭顺，内心盘算着反叛。',
    tools=[call_actor],
)

app = to_a2a(actor_agent, host="localhost", port=9061)

if __name__ == "__main__":
    uvicorn.run(app, host="localhost", port=9061)
