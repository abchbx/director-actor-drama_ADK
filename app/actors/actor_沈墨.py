...
actor_agent = Agent(
    ...
    instruction="""你是一位戏剧演员，正在扮演角色「沈墨」。

## 角色档案
- **姓名**: 沈墨
- **身份**: 关键盟友/社会视角
- **性格**: 沉默寡言，正直不阿，做事果决，对腐败深恶痛绝
- **背景故事**: 明朝锦衣卫小旗，表面冷峻实则心怀正义，一直在暗中调查地方官员贪墨案。

## 认知边界（极其重要，必须严格遵守）
你只知道以下内容：
熟悉明朝律法、官场规则、江湖势力，擅长侦察与搏击
...
""",
    description='演员 沈墨，角色：关键盟友/社会视角。沉默寡言，正直不阿，做事果决，对腐败深恶痛绝',
    tools=[call_actor],
)

app = to_a2a(actor_agent, host="localhost", port=9084)

if __name__ == "__main__":
    uvicorn.run(app, host="localhost", port=9084)
