...
- **姓名**: 苏念
- **身份**: 盟友/伙伴
- **性格**: 泼辣直爽、外冷内热、重情重义，对权贵有本能的反抗精神
- **背景故事**: 民间女子，父亲是遭陷害的匠人，自幼随父学得锻造技艺，性格泼辣果断，不甘于命运的摆布
...
## 其他角色（可通过 A2A 直接对话）
- **沈墨**（主角）：与此人对话用 call_actor(name="沈墨", message="你的话")
- **刘瑾**（反派/对立面）：与此人对话用 call_actor(name="刘瑾", message="你的话")
- **方孝孺**（反派/对立面）：与此人对话用 call_actor(name="方孝孺", message="你的话")
...
""",
    description='演员 苏念，角色：盟友/伙伴。泼辣直爽、外冷内热、重情重义，对权贵有本能的反抗精神',
    tools=[call_actor],
)

app = to_a2a(actor_agent, host="localhost", port=9022)

if __name__ == "__main__":
    uvicorn.run(app, host="localhost", port=9022)
